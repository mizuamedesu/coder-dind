...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    set -eu
    mkdir -p "$HOME/.agents/skills/mdx-cli" "$HOME/.claude/skills/mdx-cli" "$HOME/.codex/skills/mdx-cli"
    cp -R /opt/mdx-skill/. "$HOME/.agents/skills/mdx-cli/"
    cp -R /opt/mdx-skill/. "$HOME/.claude/skills/mdx-cli/"
    cp -R /opt/mdx-skill/. "$HOME/.codex/skills/mdx-cli/"
    /opt/code-server/bin/code-server --auth none --bind-addr 127.0.0.1:13337 --disable-telemetry > /tmp/code-server.log 2>&1 &
    echo "Developer tools, MDX skills and code-server ready."
  EOT
...
resource "docker_image" "workspace" {
  name         = "coder-dev-tools:${substr(sha256(join("", [for f in sort(tolist(fileset("${path.module}/image", "**"))) : filesha256("${path.module}/image/${f}")])), 0, 16)}"
  keep_locally = true
  build {
